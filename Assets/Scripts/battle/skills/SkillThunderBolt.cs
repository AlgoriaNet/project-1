...
namespace battle
{
    public class SkillThunderBolt : BaseSkillController
    {
        private List<MonsterManager> affectedEnemies = new List<MonsterManager>();
        private float damageRange = 3f; // Range for thunder bolt damage
        ...
        private IEnumerator DamageDetectionCoroutine()
        {
            ...
            while (elapsed < Skill.Duration && !hasDealtDamage)
            {
                ...
                {
                    ApplyThunderBoltDamage();
                    hasDealtDamage = true;
                    break; // Thunder bolt hits once
                }
        ...
        private void ApplyThunderBoltDamage()
        {
        ...
        {
            // Thunder bolt effect after attack
        }
    }
}